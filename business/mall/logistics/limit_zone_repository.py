...
class LimitZoneRepository(busniess_model.Service):
	def get_limit_zones(self):
		...
		获取corp中所有的限定区域
		"""
		limit_zone_models = mall_models.ProductLimitZoneTemplate.select(owner_id=self.corp.id)
		datas = []
	...
	def get_limit_zone(self, limit_zone_id):
		...
		获取指定的limit zone
		"""
		limit_zone_model = mall_models.ProductLimitZoneTemplate.select(owner_id=self.corp.id, id=limit_zone_id).get()
