...
class ShipperRepository(business_model.Service):
	...
	def get_shipper(self, shipper_id):
		...
		获得指定的发货人
		"""
		shipper_model = mall_models.Shipper.select().dj_where(owner_id=self.corp.id, id=shipper_id).get()
	...
	def delete_shipper(self, shipper_id):
		...
		删除指定的发货人
		"""
		mall_models.Shipper.delete().dj_where(owner_id=self.corp.id, id=shipper_id).execute()

	def get_shippers(self):
		"""
		获取corp中所有的发货人
		"""
		shipper_models = mall_models.Shipper.select().dj_where(owner_id=self.corp.id,is_deleted=False).order_by(mall_models.Shipper.created_at)
		datas = []
