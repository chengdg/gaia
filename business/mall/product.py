...
class Product(business_model.Model):
	...
	def modify(self,args):
		...
		db_standard_model = mall_models.ProductModel.select().dj_where(
			owner_id=owner_id,
		...
		if False:
			...
			if standard_model.get('is_deleted', None):
				# 商品有单规格变为多规格时，标准规定的model置为is_deleted=True
				if not db_standard_model.is_deleted:
					# 单规格改多规格商品
				...
				mall_models.ProductModel.update(
					...
					owner_id=owner_id, product_id=product_id, name='standard'
				).execute()

			# 清除旧的custom product model
			existed_models = [product_model for product_model in mall_models.ProductModel.select().dj_where(
				owner_id=owner_id,
				product_id=product_id
			...
			for custom_model in custom_models:
				...
				if custom_model_name in existed_model_names:
					...
					product_model = mall_models.ProductModel.select().dj_where(
						product_id=product_id, name=custom_model_name)
					mall_models.ProductModelHasPropertyValue.delete().dj_where(
						model=product_model).execute()
				else:
					# model不存在，创建之
			...
			if len(to_be_deleted_model_names):
				mall_models.ProductModel.update(is_deleted=True).dj_where(
					product_id=product_id, name__in=to_be_deleted_model_names
				)

			# 处理轮播图
			mall_models.ProductSwipeImage.delete().dj_where(
				product_id=product_id
			).execute()
			...
			existed_property_ids = set([property.id for property in mall_models.ProductProperty.objects.filter(owner_id=owner_id, product_id=product_id)])
			for property in properties:
				if property['id'] < 0:
					...
					mall_models.ProductProperty.update(name=property['name'], value=property['value']).dj_where(
						owner_id=owner_id, id=property['id']
					)
			property_ids_to_be_delete = existed_property_ids - property_ids
			if property_ids_to_be_delete:
				mall_models.ProductProperty.objects.filter(
					id__in=property_ids_to_be_delete).delete()

			# 更新product
			postage_type = args['postage_type']
			if postage_type == mall_models.POSTAGE_TYPE_UNIFIED:
				postage_id = -1
				unified_postage_money = args.get(
					'unified_postage_money', '')
				if unified_postage_money == '':
					unified_postage_money = 0.0
			else:
				postage_id = 999  # args['postage_config_id']
				unified_postage_money = 0.0

			min_limit = args.get('min_limit', '0')
			...
			if purchase_price == '' or not purchase_price:
				purchase_price = 0
			is_enable_bill = int(args.get('is_enable_bill', '0'))
			is_delivery = int(args.get('is_delivery', '0'))
			...
			param = {
				...
				'promotion_title': args.get('promotion_title', '').strip(),
				'user_code': args.get('user_code', '').strip(),
				'bar_code': args.get('bar_code', '').strip(),
				'thumbnails_url': thumbnails_url,
				'detail': args.get('detail', '').strip(),
				'type': args.get('type', mall_models.PRODUCT_DEFAULT_TYPE),
				'is_use_online_pay_interface': 'is_enable_online_pay_interface' in args,
				'is_use_cod_pay_interface': 'is_enable_cod_pay_interface' in args,
				'postage_id': postage_id,
				'unified_postage_money': unified_postage_money,
				'postage_type': postage_type,
				'stocks': min_limit,
				'is_member_product': args.get("is_member_product", False) == 'on',
				# 'supplier': args.get("supplier", 0),
				# 'purchase_price': purchase_price,
				'is_enable_bill': is_enable_bill,
				'is_delivery': is_delivery,
				'buy_in_supplier': int(args.get('buy_in_supplier', False)),
				'limit_zone_type': int(args.get('limit_zone_type', '0')),
				'limit_zone': int(args.get('limit_zone_template', '0'))
			}

			if mall_type == 1:
				param['supplier'] = args.get("supplier", 0)
				param['purchase_price'] = purchase_price

			# 微众商城代码
			# if args.get('weshop_sync', None):
			#     param['weshop_sync'] = args['weshop_sync'][0]
			...
			# 	id=product_id
			# ).update(**param)
			mall_models.Product.update(**param).execute()
