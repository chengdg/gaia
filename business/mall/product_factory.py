...
class ProductFactory(business_model.Model):
	...
	def __init_base_info(self, product, args):
		...
		category_ids = base_info.get('product_category', '')

		product.category_ids = category_ids.split(',')

	def __init_models_info(self, product, args):
		models_info = args['models_info']

		is_use_custom_models = models_info['is_use_custom_models']
		if is_use_custom_models:
			...
			price = standard_model.get('price', '0.0').strip()
			weight = standard_model.get('weight', '0.0').strip()
			stock_type = int(standard_model.get(
				'stock_type', mall_models.PRODUCT_STOCK_TYPE_UNLIMIT))
			stocks = standard_model['stocks']

			user_code = standard_model.get('user_code', '').strip()
			standard_model = {
				"price": price,
				"weight": weight,
				"stock_type": stock_type,
