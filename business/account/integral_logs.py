...
class IntegralLogs(business_model.Model):
	...
	def get(args):
		...
		"""
		integral_logs = IntegralLogs(args['webapp_user'])
	...
	def __get_integral_logs(self):
		...
		followers = Member.from_tokens({'token': list(followers_token)})
		...
		for integral_log in integral_logs:
			...
			if integral_log.follower_member_token:
				...
				integral = IntegralLog(webapp_user, integral_log, follower=follower)
			else:
				integral = IntegralLog(webapp_user, integral_log, follower=None)
